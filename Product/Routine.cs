        ...
        public static void Pulse()
        {
            ...
            while (_launched)
            {
                try
                {
                    if (!Products.InPause)
                    {
                        if (!ObjectManager.Me.IsDeadMe && Fight.InFight && ObjectManager.Me.Target.IsNotZero() && ObjectManager.Target.GetDistance < 4.0f)
                        {
                            Move.Forward(Move.MoveAction.DownKey);

                            timeStrafe = Others.Random(100, 200);
